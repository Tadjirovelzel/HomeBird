/*
  Based on ESP32-CAM-Video-Recorder-junior by https://github.com/jameszah/ESP32-CAM-Video-Recorder-junior
...
namespace video{
    ...
    int avi_length = 3;              // length in seconds of the video clip
    ...
    void major_fail() {
        ...
        for  (int i = 0;  i < 10; i++) {                 // 10 loops or about 50 seconds then reboot
    ...
    void start_avi(const char* fname) {
        ...
        if (avifile != NULL)  {
            ...
            Serial.println("Could not open avifile");
            major_fail();
        }
        ...
        if (idxfile != NULL)  {
            ...
            Serial.println("Could not open idxfile");
            major_fail();
        }
    ...
    void record(const char* aviname) {
        while(true){
            ...
            if (frames == 1 ) {                                 // start the avi
            ...
            } else if (ftell(avifile) > 4750000 || millis() > (avi_start_time + avi_length * 1000)) {       // end the avi
                ...
                another_save_avi(fb_curr);                      // put next frame in avi
                esp_camera_fb_return(fb_curr);                  // get rid of current frame
    ...
    void initialize(){
        ...
        Serial.println("                                    ");
        Serial.println("-------------------------------------");
        Serial.println("ESP32-CAM-Video-Recorder-junior");
        Serial.println("-------------------------------------");
        ...
        // Testfile 1
        FILE* testfile = fopen("/spiffs/test", "w");
        if (testfile == NULL){
            Serial.println("Cannot open testfile 1 on SPIFFS");
        } else{
            Serial.println("SPIFFS initialized succesfully");
        }
        fclose(testfile);
        remove("/spiffs/test");
        ...
        if(!file){
            Serial.println("Cannot open testfile 2 on SPIFFS");
        } else{
            Serial.println("SPIFFS initialized succesfully");
        }
        file.close();    
    ...
    void clear(const char* file_path){
        remove(file_path);
        Serial.print("Removed file: "); Serial.println(file_path);
    }
